...
def random_initial(constraints, n, ciphertext, alphabets, symbol_cipher):
	...
	for r in range(1, R+1):
		...
		#e.g. 	current_constraints  =  [('I', [0]), ('L', [1]), ('S', [2]), ('K', [3]), ('T', [4, 7]), ('E', [5, 9]), ('R', [6]), ('H', [8])]
		...
		if initScore < bestInitScore:
			...
			print "corresponding putative plaintext = " + get_putative_plaintext(ciphertext, bestInitKey)
...
def outer_swap(constraints, mi, mj):
	constraints[mi][1] = constraints[mi][1] + 1  
	constraints[mj][1] = constraints[mj][1] - 1  
	return constraints
'''
...
def outer_hill_climb(ciphertext):
	...
	for i in range(0, 25):
		for j in range(0, 26-i):
			...
			#outerswap: m'j, m'j+i
			constraints_mprime = outerswap(constraints_mprime, j, j+i)
			...
			if score<bestScore:
				...
				constraints_mprime = outerswap(constraints_mprime, j+i, j)
				score = random_initial(constraints_mprime, n, ciphertext, alphabets, symbol_cipher)	
				if score < bestScore:
					constraints_m = constraints_mprime
					bestScore = score
			...
			print "corresponding putative plaintext = " + get_putative_plaintext(ciphertext, bestKey)
