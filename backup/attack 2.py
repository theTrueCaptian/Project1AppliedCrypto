...
return score
'''
def inner_hill_climb(K, digram_putative_plaintext, n, ciphertext, alphabets):
	#innerScore = d(Dc , E)
...
def outer(ciphertext):
	...
	for r in range( R-1):
		...
		K = [(letter[0], sym) for letter in current_constraints for sym in letter[1] ] 
		if DEBUG_OUTER:
		...
		initScore = inner_hill_climb(K, digram_putative_plaintext, n, ciphertext, alphabets )
